...
if __name__ == '__main__':
    ...
    theta_1 = LRG.LR_gradient(X_tr, y_tr,alpha=0.01, tol=1e-3,max_iter=1e6) # 梯度下降解法 零数据直接跑的准确率为0.715
    time_end = time.time()
    ...
    time_start = time.time()
    theta_2 = LRG.LR_gradient_norm(X_tr, y_tr, alpha=0.01,Lambda=0.5,tol=1e-3, max_iter=1e6)  # 梯度下降解法 零数据直接跑的准确率为0.715
    time_end = time.time()
    ...
    # minitype_data(X_tr,y_tr,X_test,y_test,theta_1, theta_1)
    ...
    print("Logistic Regression 梯度下降算法")
    time_start = time.time()
    theta_4 = LRG.LR_gradient(X_tr, y_tr,alpha=0.001, tol=1e-3,max_iter=1e5) # 梯度下降解法 零数据直接跑的准确率为0.715
    time_end = time.time()
    ...
    print("Logistic Regression 牛顿迭代法")
    time_start = time.time()
    theta_5 = LRN.LR_newton(X_tr, y_tr, tol=1e-3, max_iter=1e5)  # 梯度下降解法 零数据直接跑的准确率为0.715
    time_end = time.time()
